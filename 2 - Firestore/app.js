...
function criarCard() {
    ...
    /**
     * .collection('colecao'): Referenciar a coleção
     ...
     * - Com batch, ou são gravadas todas as operações, ou nenhuma é gravada.
     */
...
function deletar(id) {
    ...
    // .delete(): deleta documentos da coleção. OBS: Pode ser usado APENAS em documentos.
    firebase.firestore().collection('cards').doc(id).delete().then(() => {
        card.remove();
    });

    // Para remover uma propriedade do documentos, podemos dar um update() e passamos no objeto
    // a propriedade que será excluida e chamamos o método de .delete() vindo de firebase.firestore.FieldValue
...
function curtir(id) {
    ...
    /**
     * .update({dados}): Atualiza todos os dados passados por parametro. OBS: Pode ser usado apenas em docs.
     */
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //firebase.firestore().collection('cards').get().then(snapshot => {
        ...
        //retorna um array com as mudanças que essa coleçao sofreu desde a ultima leitura
        // snapshot.docChanges
    ...
    //     // Usar desta forma é equivalente ao .on('value') do Realtime Database
     ...
     * não é possivel realizar consultas em coleções aninhadas, como:
     * curso where <- colecao
     * -CCO <- documento
     * --alunos where <- coleção aninhada
     * para fazer algo parecido, deve-se estruturar o banco de forma correta para que não precise usar um where dentro de um where
     * ou realizar um workaround, como um filter do javascript ou um reduce por exemplo
     */  
    ...
    /**
     * Ordenação
     *  .orderBy(campo, ordenação): Ordena pelo campo e pelo tipo de ordenação passados, tipo não origatorio
     *  OBS: ao usar juntamente do .where() deve-se ordenar pelo mesmo campo
     */
    // firebase.firestore().collection('cards').orderBy('curtidas', 'desc').get().then(snapshot => {
    //     snapshot.docs.forEach(card => {
    //         adicionaCardATela(card.data(), card.id);
    //     });
    // });
    ...
    firebase.firestore().collection('cards').limit(3).get().then(snap => {
        ...
        firebase.firestore().collection('cards').orderBy('idade').startAt(startAt).get().then(snapshot => {
            snapshot.docs.forEach(card => {
                adicionaCardATela(card.data(), card.id);
            });
         });
